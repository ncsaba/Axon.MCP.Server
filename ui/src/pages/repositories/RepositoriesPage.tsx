...
export default function RepositoriesPage() {
  ...
  const handleSyncClick = async (row: RepositoryResponse) => {
    try {
      ...
      setToastMessage({ message: `Sync triggered for ${row.name}`, type: ToastTypeEnum.success });
      await fetchRepositories();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to sync repository");
      setToastMessage({ message: "Failed to sync repository", type: ToastTypeEnum.error });
    } finally {
      setSyncingId(null);
    }
  };
  ...
  const handleCreateRepository = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setToastMessage({ message: `Repository ${created.name} created and queued for sync`, type: ToastTypeEnum.success });
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setToastMessage({ message: `Repository "${deletingRepo.name}" deleted successfully`, type: ToastTypeEnum.success });
      setDeletingRepo(null);
      setSelectedIds((prev) => prev.filter((id) => id !== deletingRepo.id));
      await fetchRepositories();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to delete repository");
      setToastMessage({ message: "Failed to delete repository", type: ToastTypeEnum.error });
    } finally {
  ...
  const handleUpdateRepository = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setToastMessage({ message: `Repository "${payload.name}" updated successfully`, type: ToastTypeEnum.success });
      setFormState(initialFormState);
      setEditingRepo(null);
      await fetchRepositories();
      setShowForm(false);
      ...
      setToastMessage({ message: "Failed to update repository", type: ToastTypeEnum.error });
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleBulkSync = async () => {
    ...
    try {
      ...
      setToastMessage({ message: `Bulk sync triggered: ${result.jobs_created} jobs created`, type: ToastTypeEnum.success });
      setSelectedIds([]);
      await fetchRepositories();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to trigger bulk sync");
      setToastMessage({ message: "Bulk sync failed", type: ToastTypeEnum.error });
    } finally {
  ...
  const handleBulkEnrich = async () => {
    ...
    try {
      ...
      setToastMessage({ message: `Enrichment triggered for ${selectedIds.length} repositories`, type: ToastTypeEnum.success });
      setSelectedIds([]);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to trigger enrichment");
      setToastMessage({ message: "Enrichment trigger failed", type: ToastTypeEnum.error });
    } finally {
      setBulkOperationInProgress(false);
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      setToastMessage({ message: `Bulk delete completed: ${result.removed_count} repositories deleted`, type: ToastTypeEnum.success });
      setSelectedIds([]);
      setShowBulkDeleteConfirm(false);
      await fetchRepositories();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to delete repositories");
      setToastMessage({ message: "Bulk delete failed", type: ToastTypeEnum.error });
    } finally {
      setBulkOperationInProgress(false);
    }
  };
    ...
    <div className={styles.repositories_container}>
      <div className={styles.repositories_header}>
        ...
        <div className={styles.repositories_actions}>
          {selectedIds.length > 0 && (
            <>
              <button className={styles.primary_button} onClick={handleBulkSync} disabled={bulkOperationInProgress}>
                Sync Selected ({selectedIds.length})
              </button>
              <button className={styles.primary_button} onClick={handleBulkEnrich} disabled={bulkOperationInProgress}>
                Enrich Selected ({selectedIds.length})
              </button>
              <button className={styles.danger_button} onClick={() => setShowBulkDeleteConfirm(true)} disabled={bulkOperationInProgress}>
      ...
      {repositories.length === 0 ? (
        <div className={styles.empty_state}>
          ...
          <p className={styles.empty_state_hint}>Repositories will appear here once they are synced from GitLab.</p>
        </div>
      ) : (
        <>
          <RepoTable
      ...
      {syncingId !== null && <div className={styles.syncing_overlay}>Sync queued for repository #{syncingId}</div>}
      ...
      <GitLabDiscoveryModal
        ...
        onSuccess={() => void fetchRepositories()}
      />
    </div>
  );
}
